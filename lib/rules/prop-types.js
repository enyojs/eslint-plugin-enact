...
module.exports = {
  meta: {
    docs: {
      description: 'Prevent missing props validation in a Enact component definition',
  ...
  create: Components.detect(function(context, components, utils) {
    ...
    function getPropertyName(node) {
      ...
      var isInClassComponent = utils.getParentES5Component();
    ...
    function markPropTypesAsUsed(node, parentNames) {
      ...
      switch (node.type) {
        ...
        case 'FunctionExpression':
          type = 'destructuring';
          properties = [];
          for(var n=0; n<node.params[0].properties.length; n++) {
            var prop = node.params[0].properties[n];
    ...
    function reportUndeclaredPropTypes(component) {
      ...
      for (var i = 0, j = component.usedPropTypes.length; i < j; i++) {
        ...
        if (
          ...
          component.computedProps.indexOf(allNames[0])>=0
    ...
    function markDestructuredFunctionArgumentsAsUsed(node) {
      ...
      if (isKindRender(node) || (destructuring && components.get(node))) {
