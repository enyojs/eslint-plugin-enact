  ...
  create: Components.detect(function(context, components, utils) {
    ...
    function isAnnotatedFunctionPropsDeclaration(node) {
      if (node && node.params && node.params.length) {
        var tokens = sourceCode.getFirstTokens(node.params[0], 2);
    ...
    function getKeyValue(node) {
      if (node.type === 'ObjectTypeProperty') {
        var tokens = sourceCode.getFirstTokens(node, 1);
     ...
     * @param {Node} node The node being marked.
     */
    ...
    function markPropTypesAsDeclared(node, propTypes) {
      ...
      switch (propTypes && propTypes.type) {
        ...
        case 'MemberExpression':
          ...
          break;
        case 'Identifier':
          var variablesInScope = variable.variablesInScope(sourceCode, node);
